...
# you can use the same pattern for many problems to search a space of choices. Rather than looking at the whole array, our convention is to consider the part of the array starting at index start and 
# continuing to the end of the array. The caller can specify the whole array simply by passing start as 0. No loops are needed the recursive calls progress down the array. 
...
# Given an array of ints, is it possible to choose a group of some of the ints, beginning at the start index, such that the group 
# sums to the given target? However, with the additional constraint that all 6's must be chosen. (No loops needed.)
...
# Given an array of ints, is it possible to choose a group of some of the ints, such that the group sums to the given target with these additional constraints: all multiples of 
# 5 in the array must be included in the group. If the value immediately following a multiple of 5 is 1, it must not be chosen. (No loops needed.)
def groupSum5(start, nums, target):
    ...
    if(nums[start] % 5 == 0):
        if nextStart < len(nums) and nums[nextStart] == 1:
            nextStart += 1

    if groupSum5(nextStart, nums, target - nums[start]):
        return True
  
    if nums[start] % 5 != 0 and groupSum5(nextStart, nums, target):
        return True
...
# if there are numbers in the array that are adjacent and the identical value, they must either all be chosen, or none of them chosen. For example, with the array 
...
def groupSumClump(start, nums, target):
    ...
    while nextStart < numLen and nums[nextStart] == num:
        nextStart +=1
    
    total = (nextStart - start) * num
    return groupSumClump(nextStart, nums, target - total) or groupSumClump(nextStart, nums, target)
    
# Given an array of ints, is it possible to divide the ints into two groups, so that the sums of the two groups are the same. Every int must be in one group 
# or the other. Write a recursive helper method that takes whatever arguments you like, and make the initial call to your recursive helper from splitArray(). (No loops needed.)
def splitArray(nums):
    
    return splitArrayHelper(nums, 0, 0)

def splitArrayHelper(num, i, target):

    if i == len(num):
        return (target == 0)

    if splitArrayHelper(num, i + 1, target + num[i]):
        return True

    return splitArrayHelper(num, i + 1, target - num[i])

# Given an array of ints, is it possible to divide the ints into two groups, so that the sum of one group is a multiple of 10, and the sum of the other group 
# is odd. Every int must be in one group or the other. Write a recursive helper method that takes whatever arguments you like, and make the initial call to your 
# recursive helper from splitOdd10(). (No loops needed.)
def splitOdd10(nums):

    return splitOdd10Helper(nums, 0, 0, 0)
  
def splitOdd10Helper(num, i, n1, n2):

    if i == len(num):
        return (n1 % 2 == 1 and n2 % 10 == 0 or n2 % 2 == 1 and n1 % 10 == 0)

    if splitOdd10Helper(num, i + 1, n1 + num[i], n2):
        return True

    return splitOdd10Helper(num, i + 1, n1, n2 + num[i])
...
def split53Helper(num, i, x):

    if i == len(num):
        return (x == 0)
    
    if num[i] % 5 == 0:
        return split53Helper(num, i + 1, x + num[i])
    
    if num[i] % 3 == 0:
        return split53Helper(num, i + 1, x - num[i])

    if split53Helper(num, i + 1, x + num[i]):
        return True
